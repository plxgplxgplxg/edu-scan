...
export class SubmissionsService {
  ...
  async findOneWithScore(id: string, requestUser: { id: string; role: string }) {
    ...
    if (requestUser.role === Role.STUDENT && submission.studentId !== requestUser.id) {
  ...
  async manualOverride(id: string, dto: UpdateSubmissionOverrideDto) {
    ...
    if (dto.studentCode) {
      ...
      const student = await this.prisma.user.findUnique({ where: { studentCode: dto.studentCode } });
      if (student) {
        updateData.studentId = student.id;
      }
    }
    
    if (dto.resolvedTestCode) updateData.resolvedTestCode = dto.resolvedTestCode;
    if (dto.resolvedVariantId) updateData.resolvedVariantId = dto.resolvedVariantId;
    ...
    const updatedSubmission = await this.submissionsRepository.update(id, updateData);

    if (dto.details && dto.details.length > 0) {
      for (const detail of dto.details) {
        await this.submissionsRepository.updateSubmissionDetail(id, detail.questionNumber, {
